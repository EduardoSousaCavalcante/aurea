        ...
        <div class="row justify-content-center g-4">
                ...
                <div class="card h-100 shadow-sm d-flex flex-column p-3">
                    ...
                    <form action="{{ route('cards.update', $card->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="titulo" id="titulo" class="form-control" value="{{ $card->titulo }}">
                        </div>

                        <div class="mb-3">
                            <label for="descricao">Descrição</label>
                            <textarea name="descricao" id="descricao" class="form-control">{{ $card->descricao }}</textarea>
                        </div>

                        <div class="mb-3">
                            <label for="link">Link</label>
                            <input type="url" name="link" id="link" class="form-control" value="{{ $card->link }}">
                        </div>

                        <button type="submit" class="btn btn-primary">Atualizar Card</button>
                    </form>
                </div>
            </div>
            ...
            <div class="col-12 col-md-6 col-lg-3 mb-4">
                <div class="card h-100 d-flex flex-column" id="cardPreview">
                    <img src="{{ asset('images/'.$card->img) }}" class="card-img-top" id="previewImg" alt="Imagem do card">
                    <div class="card-body d-flex flex-column">
                        <h5 class="card-title" id="previewTitulo">{{ $card->titulo }}</h5>
                        <p class="card-text" id="previewDescricao">{{ $card->descricao }}</p>

                        <div class="d-flex justify-content-between align-items-center mt-auto mb-1">
                            <a href="{{ $card->link }}" class="btn btn-purple" id="previewLink">Ver mais</a>
                            <div>
                                <i class="bi bi-heart-fill text-danger me-2"></i>{{ $card->curtidas }}
                                <i class="bi bi-eye-fill ms-3"></i> {{ $card->views }}
                            </div>
                        </div>
                    </div>
                </div>
